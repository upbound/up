...
func TestCleanDirWithTilde(t *testing.T) {
	type args struct {
		dir  string
		home string
	}
	...
	cases := map[string]struct {
		...
		"TildeUsedInHome": {
			reason: "Should fully resolve home if a tilde is specified.",
			args: args{
				dir:  "~/.up/cache",
				home: "/Users/user1",
			},
			want: want{
				cleanedDir: "/Users/user1/.up/cache",
			},
		},
		"TildeNotUsedInHome": {
			reason: "Should return supplied cacheroot if a tilde is not specified.",
			args: args{
				dir:  "/Users/user2/.up/cache",
				home: "/Users/user2",
			},
			want: want{
				cleanedDir: "/Users/user2/.up/cache",
			},
		},
	}
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			got, err := CleanDirWithTilde(
				tc.args.dir,
				func() (string, error) {
					return tc.args.home, nil
				})
			if diff := cmp.Diff(tc.want.cleanedDir, got); diff != "" {
				t.Errorf("\n%s\nCleanDirWithTilde(...): -want, +got:\n%s", tc.reason, diff)
			}
