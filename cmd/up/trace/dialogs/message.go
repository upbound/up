...
func NewMessageDialog(text string) *MessageDialog {
	...
	dialog.textview.SetBorderColor(style.DialogBorderColor)

	// textview layout
	tlayout := tview.NewFlex().SetDirection(tview.FlexColumn)
	tlayout.AddItem(emptyBoxSpace(style.DialogBgColor), 1, 0, false)
	...
	dialog.layout.AddItem(emptyBoxSpace(style.DialogBgColor), 1, 0, true)
	dialog.layout.AddItem(tlayout, 0, 1, true)
	dialog.layout.AddItem(dialog.form, style.DialogFormHeight, 0, true)
